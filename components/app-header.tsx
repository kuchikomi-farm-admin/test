...
import { Menu, X, Home, User, LogOut, Settings, ChevronDown } from "lucide-react"
...
export function AppHeader({ isAdmin = false }: AppHeaderProps) {
  ...
  // Hydration safety: ensure client-only rendering for stored data or handle hydration mismatch
              ...
              <div className="absolute right-0 top-full mt-1 z-50 w-52 rounded-xl border border-[#1B3022]/10 bg-[#F8F9FA] shadow-lg py-1.5">
                ...
                <div className="border-t border-[#1B3022]/8 mt-1 pt-1">
                  <Link
                    href="/"
                    onClick={() => setUserMenuOpen(false)}
                    className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-[#1B3022]/40 hover:text-[#1B3022]/70 transition-colors"
                  >
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-[#1B3022]/10 bg-[#F8F9FA] px-4 py-3 space-y-1">
          ...
          <div className="pt-2 border-t border-[#1B3022]/10 mt-2 space-y-1">
            ...
            <Link
              href="/"
              className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-[#1B3022]/40 hover:text-[#1B3022]/70"
            >
              <LogOut className="w-4 h-4" />
              {"ログアウト"}
            </Link>
